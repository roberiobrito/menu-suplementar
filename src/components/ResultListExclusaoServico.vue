<template>
  <div class="lista-itens-cadastrados">
    ...
    <ul class="listLinks">
      <li v-for="(item, index) in this.listaServicosCadastrados"
          :key="index">
...
export default {
  props: ['listaServicosCadastrados'],
  data: () => ({
    show: true
  }),
  methods: {
    removeServico: async function (id) {
      let vm = this

      // Criando objeto para acessar metodos
      vm.servico = new ServicoExclusao(this.$resource)

      // Consumindo webService da buscaCamaranetApiServiços
      await vm.servico.ExcluiServico(id)
      alert('Servico excluído!')
      location.reload()
    }
  }
}
